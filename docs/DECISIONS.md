...
## 2026-03-27: Pixel-perfect Implementation of Kanban Board (Task 1234)

**Date:** 2026-03-27
**Agent:** Figma to Code (claude-sonnet-4-6)
**Task:** 1234 — "Me faça uma tela de board exatamente igual ao Figma"
**Figma:** `ik0Qa30O9oNUy3qelJbQO7`, node `1-2`

**Decision:** Complete fresh implementation of `index.html` as a single self-contained file. All design tokens, layout measurements, colors, typography, and spacing were taken directly from the extracted Figma data (figma-extract.json, figma-tokens.md, figma-extract-report.md). No prior code was reused.

**Design tokens applied (exact Figma values):**
- Screen: 1440x1011px desktop
- Page background: `#F5F5F5`
- Surface (header, sidebar, columns, cards): `#FFFFFF`
- Border: `#E8E8E8` (1px solid throughout)
- Brand gradient: `linear-gradient(180deg, #F4EBFF 0%, #7F56D9 113.64%)` (from Figma vector gradientStops)
- Text dark: `#1A1A1A`, medium: `#666666`, light: `#999999`
- Semantic — success: `#10B981`, warning: `#F59E0B`, error: `#EF4444`, info: `#3B82F6`
- Font: Inter (Google Fonts, weights 400/500/600/700)
- Font sizes: 11px (xs), 12px (small), 14px (body), 18px (h3), 24px (h2), 32px (h1)
- Header: height 76px, padding 16px 32px, gap 10px, fixed, z-index 100
- Sidebar: width 224px, padding 20px 0, fixed, z-index 90, border-right 1px solid #E8E8E8
- Search input: 320x44px, border-radius 999px
- Avatar group: 32x32px per avatar, -8px overlap
- Columns (x3): 378x846px each, padding 16px 20px, gap 10px between cards, border 1px solid #E8E8E8
- Cards: border-radius 8px, padding 14px 16px, shadow 0 1px 2px 0 rgba(0,0,0,0.1)
- Card avatars: 24x24px, -6px overlap, 2px white border

**Architecture choices:**
- Single self-contained `index.html` with embedded `<style>` and `<script>`
- CSS custom properties (CSS variables) for all design tokens
- Semantic HTML5 (`header`, `aside`, `main`, `section`, `article`, `nav`)
- Full ARIA attributes (`role`, `aria-label`, `aria-current`, `aria-grabbed`, `aria-modal`)
- Inline SVG icons — zero external icon dependencies
- Native HTML5 Drag-and-Drop API with `dragstart`, `dragover`, `dragenter`, `dragleave`, `drop` events
- Keyboard accessibility: Space/Enter to grab, ArrowLeft/ArrowRight to move columns, Escape to cancel
- Search filtering via `input` event on the header search field
- Add-task modal with form validation and dynamic card injection
- Column card counts updated live on every drop/add

**Column color accents (3px top bar):**
- Todo: `#3B82F6` (info/blue)
- In Progress: `#F59E0B` (warning/amber)
- Done: `#10B981` (success/green)

**Files created/modified:**
- `/tmp/fusion-worktrees/run-1774619991643/index.html` — Complete implementation (single file)
